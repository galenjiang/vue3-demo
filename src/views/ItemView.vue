...
const item = computed(
  () => list.find(([url, time]) => url.includes(id.value))!
);
async function goBack() {
  document.documentElement.classList.add("back-transition");
  const transition = transitionHelper(() => {
    router.push(`/list?from=${id.value}`);
  });
...
<template>
  <div class="m-2 grid grid-cols-6 gap-1">
    <header
      class="title col-span-6 px-1 text-4xl bg-purple-400 text-white cursor-pointer"
      @click="goBack"
    >
      <span>{{ "<" }}</span>
      <div class="title-name mx-2 inline-block">Hello</div>
    </header>
    <div class="col-span-5">
      <img :src="item[0]" class="cover w-full" />
      <time>{{ item[1] }}</time>
    </div>
    <aside class="col-span-1 overflow-auto h-screen grid gap-1">
      <h4
        @click="toDetail(img)"
